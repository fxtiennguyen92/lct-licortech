    ...
    <div class="cui__layout__content">
        <div class="cui__breadcrumbs">
            <div class="cui__breadcrumbs__path">
                ...
                <span>
                    <span class="cui__breadcrumbs__arrow"></span>
                    <strong class="cui__breadcrumbs__current">{{ $data->title }}</strong>
                </span>
            </div>
        </div>
        ...
        <div class="cui__utils__content">
            ...
            <div class="row">
                <div class="col-md-12">
                    <form id="form-data" method="post" action="{{ route('cms.post.edit', $data->id) }}"
                            ...
                            <div class="card-body">
                                <div class="form-group">
                                    ...
                                    <input class="form-control" id="blogTitle" type="text" name="title" maxlength="250"
                                        value="{{ old('title', $data->title) }}" name-validation="{{ trans('text.title') }}"
                                        data-validation="[NOTEMPTY]">
                                </div>
                                <div class="form-group">
                                    <label class="form-label">Link</label>
                                    <input class="form-control" id="blogRoute" type="text" name="route" maxlength="250"
                                        value="{{ old('route', $data->route) }}" name-validation="Link"
                                        data-validation="[NOTEMPTY]">
                                </div>
                                <div class="form-group">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="file" class="dropify" name="image"
                                        @if ($data->image) data-default-file="{{ $data->image }}" @endif />
                                </div>
                                <div class="form-group">
                                    <label class="form-label">{{ trans('text.content') }}</label>
                                    <textarea id="summernote" class="form-control" placeholder="{{ trans('text.content') }}" name="content" rows="5">{!! old('content', $data->content) !!}</textarea>
                                </div>
                                <br>
                                <h5><strong>SEO</strong></h5>
                                <div class="form-group">
                                    <label class="form-label">Tag Description</label>
                                    <textarea class="form-control" placeholder="Mô tả trang" name="meta_description" rows="2">{!! old('meta_description', $data->meta_description) !!}</textarea>
                                </div>
                                <div class="form-group">
                                    <label class="form-label">Tag Keywords</label>
                                    <textarea class="form-control" placeholder="Từ khóa" name="meta_keywords" rows="1">{!! old('meta_keywords', $data->meta_keywords) !!}</textarea>
                                </div>
                                <div class="form-group">
                                    <label class="form-label">Others</label>
                                    <textarea class="form-control" placeholder="Các thẻ meta" name="meta_tags" rows="1">{!! old('meta_tags', $data->meta_tags) !!}</textarea>
                                </div>
